...
public class FileServer implements Config, SystemInterface
{
	...
	private SslContextFactory.Server createFileServerSslContextFactory() throws IOException
	{
	...
	private ServerConnector createFileServerHttpsConnector(Server server, SslContextFactory.Server factory)
	{
	...
	private void addKeyStore(SslContextFactory.Server sslContextFactory) throws IOException
	{
		...
		if (keyStore != null && keyStore.exists())
		{
			...
			if (!StringUtils.isEmpty(properties.getProperty(ServerProperties.SERVER_CIPHER_SUITES.name)))
				sslContextFactory
						.setIncludeCipherSuites(StringUtils.stripAll(StringUtils.split(properties.getProperty(ServerProperties.SERVER_CIPHER_SUITES.name), ',')));
